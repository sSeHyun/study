...
public class PipeLineMain {
	...
	public static void main(String[] args) {
		...
		System.out.println("[Nomal] 남자회원의 평균나이:"+(sum/(members.size()-2)));
		
		//2. stream로직(pipeline)
		OptionalDouble avgAge=members.stream()              //Optional이 붙으면 null값일 경우에도 핸들링 할 수 있다 그냥 Double이면 null값일 경우 nullpointerror 발생된다.
									.filter(m->m.getGender()==Member.MALE)	//남자회원만
									//.mapToInt(m->m.getAge())				//나이만 가져오기
									.mapToInt(Member :: getAge)				//나이만 가져오기
									.average();
