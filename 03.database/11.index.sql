/* Index
   ...
   1. 인덱스란?
    ...
    2. rowid구조
       ...
       rowid는 데이터의 위치정보 즉 저장된 HDD의 주소로서 오라클에서는 총 10bytes크기의
         rowid라는 정보를 제공한다.
...
select rowid,length(rowid),empno,ename from emp;
select rowid,rowidtochar(rowid),length(rowidtochar(rowid)),empno,ename from emp;
--7369가 HDD에 저장된 메모리주소 AAAE5eAAEAAAAFcAAA
...
select * from idx_test order by 1;

create index idx_test_no on idx_test(no); --0.016s
select * from idx_test order by 1;
