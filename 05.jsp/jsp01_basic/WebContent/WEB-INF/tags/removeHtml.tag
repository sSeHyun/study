...
<!-- 
	tag파일에서 몸체 내용의 EL이나 액션태그처리결과를 사용하려면
	
	1. tag 디렉티브 body-content 속성의 값을 "scriptless"로 지정
	 1) empty      : body를 가질수가 없다.
	 2) scriptless : 스크립트(스크립트릿, 스크립표현식, 선언문)을 사용할 수 없다.
	 3) JSP        : JSP안에 들어갈 수 있는 내용이라면 무엇이든지 가능
	2. jsp:body 액션태그를 이용해서 몸체 내용을 출력하거나 EL변수로 지정한다.
		body-content의 기본값은 scriptless가 기본값이다. 
		jsp:body는 다음과 같은 형태로 사용한다.
